...
### 主な変更点
- **逆改変（復元）モードを追加**：通常変換と同じ導線で「おちびちゃん化前へ戻す」処理を選べるようにし、候補プルダウンと手動Prefab指定を共通化して操作を分かりやすくしました
- **逆改変時の安全処理を強化**：逆改変では AddMenu を新規追加しないようにしつつ、既存 AddMenu の削除判定を見直して重複名・連番サフィックスを含むケースでも意図しない残留を起こしにくくしました
- **逆改変入力の検証と保持を改善**：遅延実行前にキュー済み入力を再検証し、無効アセット選択時の状態リセットや、候補が無い場合の手動指定保持など復元ワークフローの安定性を向上させました
- **Prefab候補キャッシュ生成を最適化**：AssetDatabase クエリを整理して重複チェックを削減し、除外フォルダ判定や検索範囲の扱いを統一して候補一覧の構築コストを軽減しました
- **FaceMesh一致判定を拡張**：FaceMeshキャッシュに `Animator.avatar` の識別情報を追加し、ID不在時のフォールバック条件を限定することで、逆引き候補の誤一致を減らしました
